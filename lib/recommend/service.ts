import { recommendOfficeCatalog } from "@/data/recommend/offices";
import { purposeMappingVersion } from "@/data/recommend/task-mappings";
import { defaultTravelFallbackMinutes, recommendContractVersion } from "@/lib/recommend/constants";
import { createDemoRecommendResponse } from "@/lib/recommend/demo";
import { rankRecommendationCandidates } from "@/lib/recommend/scoring";
import { filterWaitingItemsByPurpose, getPurposeTaskMapping } from "@/lib/recommend/task-mapping";
import { getServerRuntimeConfig } from "@/lib/server/env";
import { fetchPublicDataRealtimeWaiting } from "@/lib/server/public-data";
import { ODsayApiError, getPublicTransitTravelTime } from "@/lib/server/odsay";
import type {
...
function createDefaultTravelEstimateResolver() {
  ...
  return async ({
    ...
    try {
      return await getPublicTransitTravelTime({
        apiKey: runtimeConfig.odsayApiKey ?? undefined,
        origin: request.origin,
        destination: office.coordinates,
      });
    } catch (error) {
      if (error instanceof ODsayApiError && error.code === "ODSAY_CONFIG_ERROR") {
        throw error;
      }
    ...
    recommendations: rankedCandidates.map((rankedCandidate) => {
      ...
      if (!matchedOffice) {
        throw new RecommendServiceError(
          500,
          "INTERNAL_ERROR",
          "추천 결과를 정리하는 중 내부 상태가 일치하지 않습니다.",
        );
      }
